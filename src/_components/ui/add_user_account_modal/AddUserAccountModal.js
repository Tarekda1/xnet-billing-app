...
const AddUserAccountModel = ({ open, onSubmit, onClose, edit, userAcc }) => {
	...
	return (
		<Modal open={open}>
			<Header>{edit ? `${userAcc.user.firstName} ${userAcc.user.lastName}` : 'Add User'}</Header>
			<Modal.Content>
				<Form>
					<Grid>
						<Grid.Row>
			...
			<Modal.Actions>
				<Segment style={{ float: 'right', padding: '5px' }}>
					<Button className="basicStyle" loading={submitting} icon onClick={onAddUserUserAccount}>
						<Icon name="add" /> Add
					</Button>
