...
class ISPUsersRepository {
	...
	async getISPUsers({ pageParam, pageSizeParam }) {
		let page = pageParam || 0;
		//get users from db
		const users = await this.db.User.find({ isDeleted: false }).populate('package');
		let pageSize;
		if (users && users.length > 0) {
			pageSize = users.length;
		} else {
			pageSize = pageSizeParam || (users && users.length) || this.pageSizeGlobal;
		}
		...
		const offset = page * pageSize,
			paged = users.slice(offset, offset + pageSize),
			totalPages = Math.ceil(users.length / pageSize) == 0 ? 1 : Math.ceil(users.length / pageSize);
	...
	async searchRemote(searchTerm) {
		...
		const pageResult = new pagged(normalized, 0, userAccs.length, normalized.length);
		return pageResult;
	}
