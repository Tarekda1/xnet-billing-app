...
async function createUserAccount(params) {
	...
	// 	{ $match: { billDate: { $exists: true } } },
	// 	// { $match: { user: { $exists: true } } },
	// 	//{ $unwind: '$userAcc' },
	// 	// { $match: { userName: 'xxxx' } },
	// 	{ $match: { month: new Date().getUTCMonth() + 2 } }
	// ]);
	// const userAcc = await db.UserAccount.aggregate([
	// 	{ $match: { billDate: { $exists: true } } },
	// 	{ $unwind: '$user' },
	// 	{ $project: { firstName: 1, userName: 1 } },
	// 	{ $match: { month: new Date().getUTCMonth() + 1 } }
	// ]);

	// const userAcc = await db.UserAccount.aggregate([
	// 	{ $project: { user: 1, paid: 2, comment: 3, amount: 4, month: { $month: '$billDate' } } },
	// 	{ $match: { user: params.user, month: billDate.getMonth() + 1 } }
	// ]);
	...
	if (userAcc.length > 0) {
		throw `User bill for this date:"${moment.utc(billDate, 'DD-MM-YYYY', true)}" is already added.`;
	}
